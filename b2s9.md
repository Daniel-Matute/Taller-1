      ...
      .subscribe(
        onNext = {
          case (temp1, temp2) =>
            ...
            if (diferencia > 5) {
              println(f"🚨 ALERT! Significant difference detected: $diferencia%.0f°C")
            }
    ...
    .subscribe(
      onNext = { case (temp, hum, pres) =>
        ...
        if (tempInt < 20 || tempInt > 30) {
          println(s"🚨 ALERT! Temperature out of range: ${tempInt}°C")
          hayAlerta = true
        }

        if (humInt < 40 || humInt > 60) {
          println(s"🚨 ALERT! Humidity out of range: ${humInt}%")
          hayAlerta = true
        }

        if (presInt < 950 || presInt > 1050) {
          println(s"🚨 ALERT! Pressure out of range: ${presInt} hPa")
          hayAlerta = true
        }
